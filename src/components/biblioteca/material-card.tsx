...
export function MaterialCard({ material }: MaterialCardProps) {
  return (
    <article className="flex h-full flex-col justify-between rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-5 shadow-sm transition hover:border-sky-200 hover:shadow-md">
      <div>
        <div className="flex items-start justify-between gap-4">
          <span className="inline-flex items-center gap-2 rounded-full bg-cyan-100 dark:bg-cyan-500/15 px-3 py-1 text-xs font-semibold text-cyan-800 dark:text-cyan-300">
            <FileText size={14} />
          ...
          <div className="flex items-center gap-3">
            {material.ratingsCount > 0 ? (
              <span className="inline-flex items-center gap-1 text-sm font-medium text-amber-600 dark:text-amber-400">
                ...
                <span className="text-slate-400 dark:text-slate-500">({material.ratingsCount})</span>
              </span>
            ) : null}
        ...
        <Link href={`/biblioteca/${material.id}`}>
          <h2 className="mt-4 text-lg font-semibold leading-7 text-slate-950 dark:text-white transition hover:text-sky-800 dark:hover:text-sky-300">
            {material.title}
          </h2>
        </Link>
      ...
      <div className="mt-5 flex flex-col gap-3 border-t border-slate-100 dark:border-slate-800 pt-4 sm:flex-row sm:items-center sm:justify-between">
        <Link
