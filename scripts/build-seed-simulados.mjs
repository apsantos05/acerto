/**
 * Acerte — Builder do seed de simulados (questões autorais).
 * Lê content/simulados.json e gera supabase/seed_simulados.sql (idempotente por título).
 */
...
const out = [];
out.push("-- ACERTE — Seed de simulados (questões AUTORAIS). Idempotente por título.");
out.push("-- Pré-requisito: supabase/simulados.sql (tabelas + RLS + RPCs).");
out.push("");
...
begin
  ...
  if sid is null then
    ...
    values ('${esc(sim.title)}', '${esc(sim.description)}', '${esc(sim.vestibular)}', '${esc(sim.faculty)}', ${sim.durationMinutes}, '${esc(sim.difficulty)}', ${arrLit(subjects)}, ${qCount}, 'published')
